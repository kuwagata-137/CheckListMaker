...
test('planShot — 要素矩形の採用と枠・ズーム範囲', async (t) => {
  await t.test('採用時: element / 枠(PAD 6) / ズーム(余白48＋最小480×320 中心維持)', () => {
    const p = plan();
    assert.deepEqual(p.element, [1200, 640, 100, 40]);
    assert.deepEqual(p.frame, [1194, 634, 112, 52]);
    // 100×40 → 余白込み 196×136 → 最小サイズへ中心拡張
    assert.deepEqual(p.zoom, { rect: [1010, 500, 480, 320], source: 'element' });
  });
  ...
  await t.test('大きい要素は余白48だけでズーム範囲になる（最小サイズ適用なし）', () => {
    const p = plan({ rect: [600, 300, 600, 400] }, { click: { x: 900, y: 500 } });
    assert.deepEqual(p.zoom.rect, [552, 252, 696, 496]);
  });
  ...
  await t.test('画面端の要素は枠もズームも画像内へクランプされる', () => {
    ...
    assert.deepEqual(p.zoom.rect, [0, 0, 1920, 320]);
  });
  ...
  await t.test('scaleFactor は余白・最小サイズ・PAD に効く', () => {
    ...
    assert.deepEqual(p.zoom.rect, [770, 340, 960, 640]); // 最小 480×320 ×2
  });
});

test('planShot — 不採用（クリック中心フォールバック）の条件', async (t) => {
  const FALLBACK = { rect: [990, 500, 480, 320], source: 'click' }; // click(1230,660) 中心
  ...
  await t.test('フォールバックの切り出しは画像端でクランプされる', () => {
    const p = plan({ resolved: false }, { click: { x: 1900, y: 1060 } });
    assert.deepEqual(p.zoom.rect, [1440, 760, 480, 320]);
    const q = plan({ resolved: false }, { click: { x: 10, y: 10 } });
    assert.deepEqual(q.zoom.rect, [0, 0, 480, 320]);
  });
