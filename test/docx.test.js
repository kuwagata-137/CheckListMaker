...
async function buildDocx(html, meta) {
  const buffer = await HTMLtoDOCX(html, null, {
    ...
    footer: true, pageNumber: true, font: '游明朝', fontSize: 21, complexScriptFontSize: 21, lang: 'ja-JP',
  });
...
test('3-C docx 回帰 — 後処理が生成する OOXML 骨格を固定する', async (t) => {
  ...
  await t.test('footer1.xml — PAGE / NUMPAGES 中央フッター', () => {
    assert.ok(out.footer.includes('PAGE'), 'PAGE フィールド');
    assert.ok(out.footer.includes('NUMPAGES'), 'NUMPAGES フィールド');
