...
- **改ページの目安線**（2026-09-09 追加）。紙面は常に用紙の実寸で組み、モーダルに
  入りきらない用紙（A3・B4・横向き）は `transform: scale()` で縮小して見せる。
  `max-width: 100%` で潰すと padding が 15mm/25mm のまま本文幅だけ狭まり、
  折り返し＝行数が実出力とずれてしまうため。プレビューは1枚の連続した紙なので
  `@page` の上下余白はページごとには入らず、本文は「1ページ分の本文高さ
  （用紙高 − 30mm）」で単純に積み上がる。表紙・目次は `break-after: page` で
  1ページを丸ごと使うため、その下端を次ページの起点に取り直す。
  `break-inside: avoid` による送りまでは再現しないので**あくまで目安**（バーにも明記）。
