...
- **`break-inside: avoid` の送りを再現する**（2026-09-10）。当初は1ページ分の高さで
  ...
  - 目次が2ページ以上になる場合も同じ計算で刻む。紙面の下 padding（15mm）には本文が
    無いので、そこまでは線を引かない。
  - **倍率（30〜200%）にも追従する**（2026-09-10）。倍率は `.pp-zoom` の `zoom` で掛かって
    いるので、内部座標の1ページ分は「物理の1ページ高 ÷ 倍率」になる（printToPDF の
    `scale` と同じ意味論）。`zoom` の中の `getBoundingClientRect` は Chromium の版で
    座標系が違う（M128 で標準化。Electron 31 は旧挙動）ため、既知の高さ 100px の物差しを
    本文の先頭に差して unit を実測し、内部座標に揃えてから計算する。ページ座標へ戻すのは
    線を置く瞬間だけ。
  - 線は送られた手順の上辺に密着させず **6px 浮かせて**引く（見た目の要望 2026-09-10）。
