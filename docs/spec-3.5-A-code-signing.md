...
> **ユーザー決定（2026-07-14）: 本開発は一切お金をかけない方針のため、証明書の購入は
> 見送り（無署名で配布を継続）。** 有償選択肢（OV/EV/eSigner/Certum 等）はすべて対象外。
> Microsoft Store も開発者登録に一時費用（個人 $19／法人 $99 目安）がかかるため対象外。
> 無償でできる緩和策のみ実施する:
>
> - README／インストールガイドに SmartScreen 警告時の手順（「詳細情報」→「実行」）と、
>   アプリの挙動（クリック座標のみ取得・外部送信なし・オープンソース）を明記する（一部実施済み）。
> - GitHub Releases にインストーラの SHA-256 チェックサムを添付し、改ざん確認の手段を提供する。
> - ソース公開リポジトリ＝配布元である透明性を案内する（ビルドの再現手順は既存ガイド参照）。
>
> 3.5-B（自動更新）への影響: electron-updater は無署名アプリでも動作する（署名検証は
> アプリ自体が無署名ならスキップされる）が、更新ファイルの真正性保証が弱くなる点は
> 3.5-B 設計時に改めて確認する。将来 Azure Artifact Signing が日本に開放されたら
>（月 $9.99）最小費用の再検討候補として本書を更新する。
