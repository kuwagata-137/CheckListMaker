...
> ✅ **チェックしながら進めたい方へ（おすすめ）** — この手順を **CheckListMaker
> 自身のチェックリスト**にしました。1工程ずつチェックを付けながら作業できます。
> - **アプリに取り込む**：[`templates/exe作成手順.checklist.json`](templates/exe作成手順.checklist.json)
>   を、アプリの **「インポート」** で読み込む（テンプレート型の手順書として入ります）。
> - **印刷物として見る**：[`exe作成手順.pdf`](exe作成手順.pdf)（アプリの「🖨 印刷 / PDF」出力）。
