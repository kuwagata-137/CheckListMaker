---
name: dev-policy
description: 標準の開発方針（着手前の仕様確定、PR/マージはユーザー承認ゲート、main直push禁止、ブランチ衛生、外向き・不可逆操作の独断禁止）を現在のリポジトリへ展開・適用する。新しいリポジトリのセットアップ時や、別プロジェクトにもこの方針を効かせたいときに使う。`.claude/CLAUDE.md`（人間可読の方針）と、PR の作成・マージを確認（ask）に倒す小さな PreToolUse フックをひな形から設置する。
---
...
| `.claude/settings.json` | PreToolUse フック登録（PR 作成・マージ・auto-merge を ask） |
| `.claude/hooks/dev-policy-gate.py` | 上記を判定する小さなフック（fail-open） |

> フックが見るのは **GitHub MCP の PR 作成・マージ系ツール名だけ**。`main` 直 push の機械判定は
> 入れていない（Bash コマンド文字列の正規表現マッチは誤爆・回避が多く設計が難しいため。
